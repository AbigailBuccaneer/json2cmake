...
def parsecommand(command, resolvepath, extra_dirs):
...
class CompilationDatabase(object):
    ...
    def read(self, input, mask, extra_dirs):
        ...
        for entry in database:
            ...
            if 'command' in entry.keys():
                command = freeze(
                    parsecommand(
                        entry['command'], resolve, extra_dirs))
            if 'arguments' in entry.keys():
                command = freeze(
                    parsecommand(
                        entry['arguments'], resolve, extra_dirs))

            if maskre:
                if not maskre.search(entry['file']):
                    continue
...
name of the CMake project (default: taken from Git root, or 'autogenerated')
    ...
    parser.add_argument(
        '-m', '--filemask', help="""
only include files that fit the given mask
        """
    )
    parser.add_argument(
        '--extra_dirs', help="""
comma separated list of extra include dirs to add as dependencies
        """
    )
    ...
    extra_dirs = []
    if args.extra_dirs:
        extra_dirs = args.extra_dirs.split(',')

    database = CompilationDatabase()
    database.read(args.infile, args.filemask, extra_dirs)
